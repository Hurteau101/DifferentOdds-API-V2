...
class PropBuilderMapper(BaseMapper):
    ...
    async def _market_extractor(self, session, game_ids, market_mapper, team_mapper):
        tasks = []
        league_tracker = []
    ...
    def _handle_game_markets(self, market: dict, mapping: dict, league: str, stat_name: str, raw_stat_name: str):
        ...
        for game in market.get("markets", []):
            ...
            # after_fix_removal = ["_total_runs", "_total_goals", "_total_points"]
            #
            # for fix in after_fix_removal:
            #     gfm_game_key = gfm_game_key.replace(fix, "")
    ...
    def _handle_player_markets(self, market: dict, mapping: dict, league: str, stat_name: str, seen: set,
        ...
        for player in market.get("players", []):
            ...
            for market_entry in player.get("markets", []):
                ...
                player_key = f"{player_name}_{direction}_{line}_{current_stat_name}".lower().replace(" ", "_")
    ...
    def _build_market_data(self, response: list, team_mapper: dict):
        ...
        for league, data in response:
            for market in data:
                ...
                # print("Raw Stat Name: ", raw_stat_name)
