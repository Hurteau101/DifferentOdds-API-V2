...
async def test_dfs_books(book_config):
    ...
    if mapper_dict:
        class_instance = get_class_instance(class_name=mapper_dict.class_name, class_path=mapper_dict.class_path)
        valid_run = await class_instance.run_mapper()
        if not valid_run:
            pytest.fail(f"Mapper failed for {book_config.get('book_key')}")

    book_instance = get_class_instance(class_name=book_config.get("class_name"),
                                       class_path=book_config.get("class_path"))
