...
class DFSBookBase(BookBase):
    ...
    async def run_bettorodds_external_mapper(self, session: aiohttp.ClientSession, sportsbook_data: list,
        ...
        for league, league_data in payload_data.items():
            for i in range(0, len(league_data), batch_size):
                ...
                for batch in batch_results:
                    ...
                    if batch.get("teams"):
                        mapping_dict["teams"].update(batch.get("teams"))

                    if batch.get("players"):
                        mapping_dict["players"].update(batch.get("players"))

                    if batch.get("markets"):
                        mapping_dict["markets"].update(batch.get("markets"))

                # print(batch_results)
                # valid_data = not all(data for data in batch_results)
                # print(valid_data)
                # if valid_data:
        ...
        with open("results_2.json", "w") as file:
            json.dump(mapping_dict, file, indent=2)
