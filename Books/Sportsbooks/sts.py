...
#
#
#     def fetch_league(self, sport_id, session, proxy_str, cookies):
#         response = session.post(
#             url=self.book_data.url.get("league_url"),
#             headers=self.book_data.headers,
#             json={"idMainHeader": str(sport_id), "wagerTypeValue": "1"},
#             proxy=proxy_str,
#             cookies=cookies,
#         )
#         return response.json() or {}
#
#     def fetch_market(self, market, session, proxy_str, cookies):
#         response = session.post(
...
#         proxy_str = f"http://{raw_proxy[2]}:{raw_proxy[3]}@{raw_proxy[0]}:{raw_proxy[1]}"
#
#         raw_league_data = session.post(
...
#             futures = {executor.submit(self.fetch_league, sport_id, session, proxy_str, cookies): sport_id for sport_id in sports_ids}
#             league_results = [future.result() for future in as_completed(futures)]
#
...
#
#         with ThreadPoolExecutor(max_workers=2) as executor:
#             futures = {executor.submit(self.fetch_market, market, session, proxy_str, cookies): market for market in league_ids}
...
#
#         async with aiohttp.ClientSession() as mapp_session:
#             mapped_data = await self.map_runner(session=mapp_session, sportsbook_data=sts_data)
#
