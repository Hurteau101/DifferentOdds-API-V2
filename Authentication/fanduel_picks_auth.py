...
class FanduelPicksAuth(BaseScheduler):
    def __init__(self):
        ...
        with httpx.Client(http2=True) as client:  # HTTP/2 enabled
            ...
            if response.status_code != 201:
                create_sentry_message(
                    tag_key="fanduel_picks",
                    tag_value="auth_failure",
                    message="Invalid status code received during authentication attempt.",
                    level="error"
                )
            ...
            if not auth_token:
                create_sentry_message(
                    tag_key="fanduel_picks",
                    tag_value="auth_failure",
                    message="Auth token not found after authentication attempt.",
                    level="error"
                )
            ...
            await redis_instance.store_data(
                key_name="fanduel_picks_auth_token",
                data_to_store=auth_token,
                key_expiration=43200 # 12 hours
            )
